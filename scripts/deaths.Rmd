...
```{r}
deaths_by_year <- deaths %>% filter(grepl("#", icd_10_113_cause_list)) %>%  group_by(year) %>% summarise(total_deaths = sum(num_deaths, na.rm=T))

deaths %>% filter(grepl("#", icd_10_113_cause_list)) %>% count(is.na(num_deaths)) # 2,862 suppressed values, so there is an possible error of atleaet 9 * 2,862 = 25,758
...
```{r}
deaths_missouri <- deaths %>% filter(grepl("#", icd_10_113_cause_list)) %>%  filter(state=="Missouri") %>% group_by(icd_10_113_cause_list) %>% summarise(total_deaths = sum(num_deaths, na.rm=T)) %>% arrange(desc(total_deaths))
...
deaths %>% 
  filter(grepl("#", icd_10_113_cause_list), year==2020) %>%  
  group_by(state, num_pop) %>% 
  summarise(total_deaths = sum(num_deaths, na.rm=T)) %>% 
...
deaths %>% group_by(icd_10_113_cause_list) %>% filter(grepl("suicide|firearm|poisoning", icd_10_113_cause_list)) %>% summarise(total_deaths=sum(num_deaths, na.rm=T))
...
```{r}
deaths %>% filter(grepl("#", icd_10_113_cause_list)) %>% group_by(year,icd_10_113_cause_list) %>%  summarise(total_deaths=sum(num_deaths, na.rm=T)) %>% arrange(desc(total_deaths))
```
